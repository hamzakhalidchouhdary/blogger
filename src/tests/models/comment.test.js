...
describe('Comment Model', function(){
  ...
  describe('Get Comment by Id', function() {
    beforeEach(async function() {
      this.comment = await CommentFixtures.createComment('test comment', this.article.id, this.user.id);
    });
  ...
  describe('Modify Comment', function() {
    ...
    it('should update comment', async function(){
      await CommentModel.modify("updated comment", this.comment.id, this.user.id);
      const updatedComment = await CommentFixtures.getCommentById(this.comment.id);
      expect(updatedComment.content).to.equal("updated comment");
    });
    it('should not update other user comment', async function(){
      const otherUser = await UserFixtures.createUser({ role: USER_ROLES.ADMIN });
      await CommentModel.modify("updated comment", this.comment.id, otherUser.id);
      const updatedComment = await CommentFixtures.getCommentById(this.comment.id);
  ...
  describe('Remove Comment', function(){
    ...
    it('should delete user comment', async function() {
      ...
      const deletedComment = await CommentFixtures.getCommentById(this.comment.id);
